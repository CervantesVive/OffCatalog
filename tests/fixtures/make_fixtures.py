...
def make_silent_mp3(path: Path, duration_seconds: float) -> None:
    subprocess.run(
        ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=44100:cl=mono",
         "-t", str(duration_seconds), "-q:a", "9", str(path)],
...
if __name__ == "__main__":
    ...
    tag(plain, artist="Depeche Mode", title="Enjoy the Silence", album="Violator",
        tracknumber="3", date="1990")

    live = FIXTURES / "live_version.mp3"
    make_silent_mp3(live, 4.0)
    tag(live, artist="Depeche Mode", title="Enjoy the Silence (Live)", album="101",
        tracknumber="1", date="1989")
