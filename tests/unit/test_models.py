from offcatalog.models import RawTags, LocalTrack, compute_fingerprint


def make_raw(**overrides) -> RawTags:
    base = dict(
        artist="Depeche Mode", title="Enjoy the Silence", album="Violator",
        album_artist=None, track_number="3", disc_number=None, year="1990",
        isrc="GBAYE9000212", musicbrainz_track_id=None, musicbrainz_recording_id=None,
    )
...
def test_fingerprint_changes_when_tag_changes():
    ...
    fp2 = compute_fingerprint(make_raw(title="Enjoy the Silence (Remastered)"), duration_seconds=258.0, file_size=8_000_000)
    assert fp1 != fp2
...
def test_local_track_holds_raw_and_normalized_separately():
    track = LocalTrack(
        id="t1", path="/music/a.mp3", filename="a.mp3", raw=make_raw(),
        ...
        duration_seconds=258.0, year=1990, isrc="GBAYE9000212",
        musicbrainz_track_id=None, musicbrainz_recording_id=None,
