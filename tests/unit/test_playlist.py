...
def _seed_track(conn, path, state):
    ...
    track = LocalTrack(
        ...
        title="t", album=None, version_qualifiers=[], track_number=None, disc_number=None,
        duration_seconds=200.0, year=None, isrc=None, musicbrainz_track_id=None,
        musicbrainz_recording_id=None, fingerprint=path,
    )
    file_id = upsert_file(conn, path, 1.0, 1, path)
    upsert_local_track(conn, file_id, track)
    result = MatchResult(state=state, score=0.0, reason="test", candidate=None, all_candidates=[])
...
def _seed_track_two_providers(conn, path, state_a, state_b):
    ...
    record_check_result(
        ...
        MatchResult(state=state_a, score=0.0, reason="test", candidate=None, all_candidates=[]),
    )
    record_check_result(
        conn, track.id, "spotify",
        MatchResult(state=state_b, score=0.0, reason="test", candidate=None, all_candidates=[]),
    )
    return track.id


def test_unavailable_everywhere_excludes_track_available_on_one_of_two_providers(tmp_path):
    conn = get_connection(str(tmp_path / "t.db"))
    _seed_track_two_providers(conn, "/mixed_available.mp3", AvailabilityState.AVAILABLE, AvailabilityState.UNAVAILABLE)
...
def test_unavailable_everywhere_excludes_track_errored_on_one_of_two_providers(tmp_path):
    conn = get_connection(str(tmp_path / "t.db"))
    _seed_track_two_providers(conn, "/mixed_error.mp3", AvailabilityState.ERROR, AvailabilityState.UNAVAILABLE)
...
def test_unavailable_everywhere_includes_track_unavailable_on_both_of_two_providers(tmp_path):
    conn = get_connection(str(tmp_path / "t.db"))
    track_id = _seed_track_two_providers(
        conn, "/both_unavailable.mp3", AvailabilityState.UNAVAILABLE, AvailabilityState.AMBIGUOUS
    )
...
def test_unavailable_everywhere_requires_all_providers_checked_and_none_available(tmp_path):
    conn = get_connection(str(tmp_path / "t.db"))
    unavailable_id = _seed_track(conn, "/unavailable.mp3", AvailabilityState.UNAVAILABLE)
...
def test_playlist_cli_writes_file(tmp_path):
    ...
    result = runner.invoke(app, ["playlist", "--db", db_path, "--output", str(output_path)])
